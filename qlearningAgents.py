...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        predict = self.computeActionFromQValues(nextState)
        features = self.featExtractor.getFeatures(state, action, predict)

        sum = 0
        for feature, value in features.iteritems():
            sum += 0.1 + self.weights[feature] * value

        newValue = reward + self.discount * self.computeValueFromQValues(nextState)
        # oldValue = self.getQValue(state, action)
        oldValue = sum
        difference = newValue - oldValue
        self.values[(state, action)] = oldValue + self.alpha * difference

        # features = self.featExtractor.getFeatures(state, action)
        for feature, value in features.iteritems():
          self.weights[feature] += 0.2 * difference * features[feature]
        print self.weights
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        """
           Should update your weights based on transition
        """
        newValue = reward + 0.8 * self.computeValueFromQValues(nextState)
        oldValue = self.getQValue(state, action)
        difference = newValue - oldValue
