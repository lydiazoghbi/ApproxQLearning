...
def setup(gridworld, title = "Gridworld Display", size = 120):
    ...
    screen_height = (grid.height - 0.25) * GRID_SIZE + MARGIN * 2
...
def drawSquare(x, y, val, min, max, valStr, action, isObstacle, isTerminal, isCurrent):
    ...
    square_color = getColorred(6, 0, 10)

    if x == 5 & y ==5:
        square_color = getColorgreen(6, 0, 10)
    ...
    if not isObstacle:
        text( (screen_x, screen_y), text_color, valStr, "Courier", -25, "bold", "c")
...
def getColorred(val, minVal, max):
    r = 0.75
    g = 0
    return formatColor(r,g,0.0)

def getColorgreen(val, minVal, max):
    r = 0
    g = 0
    return formatColor(r,0.75,0)
